...
ggplot(mtcars, aes(x = mpg, y = wt, color = gear)) +
  geom_point() +
  labs(title = "Miles per Gallon vs. Weight by Gears", 
       x = "Miles per Gallon", 
       y = "Weight")

#' Note how the colors are not what we would expect. A car does not have 3.5
#' gears, for instance, and a continuous scale is therefore not appropriate. This is
#' because the gear variable is not a factor by default, and ggplot2 treats it
...
#' Now we have three distinct levels of the gears, as we would expect in reality.
#' This is because the gear variable is now treated as a factor, and ggplot2
#' maps the colors to the levels of the factor.
#' 
